...
export const convertResponseModelUser = (user: User): User => {
  ...
  if (user.userSettingList) {
    for (const userSetting of user.userSettingList) {
      (setting as any)[camelCase(userSetting.key)] = userSetting.value;
    }
  }
...
export const initialUserState = async () => {
  ...
  const { data } = (await api.getMyselfUser(store.getState().user.user?.openId ?? "")).data;
  if (data) {
...
export const useUserStore = () => {
  ...
  return {
    ...
    upsertUserSetting: async (key: string, value: any) => {
      ...
      await doSignIn(store.getState().user.user?.openId ?? "");
    },
